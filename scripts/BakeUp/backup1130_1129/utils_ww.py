...
#===============================================================MODIFY
'''����ɫʶ��:
1. �滻'det_color(img)'����������Ϊ'det_allKinds_color(img)'������
2. �滻'debug_image(img)'����������Ϊ' debug_allKindsCol_img(img)'������
3. 'shift_camera()'��'my_pose.ry = -90 / 1000'��ʾ������צ��y���,�����ƶ�9cm
'''
# color_dict={
#     'blue':{'hsv_l':np.array([98,63,164]),
#             'hsv_h':np.arrayy([26 ,158 ,223]),
...
#     'yellow':{'hsv_l':np.arra70, 255 ,175])}, 
#     'orange':{'hsv_l':np.array([ 8 ,150 ,174]),
#             'hsv_h':np.array([25 ,215, 240])}, 
# }
color_dict = {
    # "square": {"hsv_l": np.array([0, 66, 145]), "hsv_h": np.array([255, 255, 255])},
    "red": {"hsv_l": np.array([0, 53, 76]), "hsv_h": np.array([10, 255, 230])},
    "blue": {"hsv_l": np.array([97,112,98]), "hsv_h": np.array([142,255,255])},
    # "pink": {"hsv_l": np.array([150, 14, 216]), "hsv_h": np.array([180, 56, 255])},
  
    # "yellow": {"hsv_l": np.array([13, 82, 223]), "hsv_h": np.array([35, 255, 255])},
    # "green": {"hsv_l": np.array([45, 53, 163]), "hsv_h": np.array([75, 215, 255])},
...
# def det_rect(mask,size_threshold):
...
def debug_img(img):
    ...
    for color_name, hsv_range in zip(color_list, hsv_mask_list):      
        ...
        size_threshold = img.shape[0] * img.shape[1] * 0.05
        ...
        for cnt in contours:
            ...
            if area < size_threshold:
                ...
                if res:
                    ...
                    break
        if found:
            print(f"Detected Mask for {color_name}:")            
            break
        else:
            print(f"33333333333333333333333333fail to Detected Mask for {color_name}:")
...
def det_color(img):
    ...
    for color_name, hsv_range in zip(color_list, hsv_mask_list):
        ...
        print("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$")
        print("len col list=",len(hsv_mask_list))
        for cnt in contours:
            area = cv2.contourArea(cnt)
            if area < size_threshold:
                found = False
                ...
                if res :
                    ...
                    print(f"111111111111111111Fail to Detected Mask for {color_name}:")
                    continue
        if found:
            print(f"22222222222Detected Mask for {color_name}:")
