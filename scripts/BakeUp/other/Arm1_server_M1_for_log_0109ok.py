...
def grab_arm1_1_1_back(pre_point):
    ...
    utils.run_point(492.987,-486.749,730.369,-87.166,1.779,-136.289,50.0) 
    #time.sleep(1)
...
def grab():
    ...
    try:
        ...
        if RUNNING:
            ...
            thread.start()
            # time.sleep(2.0)
            Log_Arm1_server_print('demo start')

            Log_Arm1_server_print("going to standby point......")
            utils.go_back_home2()  
            Log_Arm1_server_print("wait 3s for catching the cube,then release grap")
            # time.sleep(3)
            Log_Arm1_server_print('end go home')
            ...
            #=============================================================infection point
            Log_Arm1_server_print("going to grab ready point from rmf......")
            param_value["speed"]=50.0
            ...
            Log_Arm1_server_print('adjusting grab rotate...... ')
            lock.acquire()
            ...
            # time.sleep(1)
            time.sleep(0.1)
            Log_Arm1_server_print("after rotate")
            ...
            #     utils.go_back_home2()
            #     Log_Arm1_server_print("go home for 2 nd recognize")s
            #     lock.acquire()
            #     image_cb = None
            #     lock.release()
